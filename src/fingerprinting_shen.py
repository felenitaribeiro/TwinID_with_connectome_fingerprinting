...
for i in range(0, 246):
    ...
    shen_matrices_r1.append(np.loadtxt(rel_path_1, delimiter=','))
    # Rest2
    rel_path_2 = './../data/Rest2/All_MZ_R2/' + lines[
        i] + '/mat_conn_finn_r2.csv'
    shen_matrices_r2.append(np.loadtxt(rel_path_2, delimiter=','))
...
ax = sns.heatmap(shen_matrices_r1[0],
                 cbar_kws={'label': 'Fischer z-transformed correlation'},vmax=1,vmin=-1)
...
for i in range(0, len(corr_network['n0_r1xr2'])):
    ...
    for m in range(0, len(corr_network['n0_r1xr2'])):
        corr_row.append(
            pearsonr(shen_matrices_r1[i].reshape(-1),
                     shen_matrices_r2[m].reshape(-1))[0])
...
for i in range(0, len(corr_network['n0_r1xr2'])):
    ...
    for m in range(0, len(corr_network['n0_r1xr2'])):
        corr_row.append(
            pearsonr(shen_matrices_r2[i].reshape(-1),
                     shen_matrices_r1[m].reshape(-1))[0])
...
for i in range(0, 10000):
    ...
    accuracies_list.append(
        np.mean(fun.accuracy(corr_matrices['n0_SI'], permuted_ID, ID_individuals)))
...
corr_distributions = {'list_of_corr_DZ_shen': [], 'list_of_corr_MZ_shen': [],
                      'list_of_corr_SI_shen': [],
                      'list_of_corr_nonrelated_shen': []}
for i in range(0, 246):
    for j in range(0, len(corr_network['n0_r1xr2'])):
        if [int(ID_individuals[i]), int(ID_individuals[j])] == twin_pairs_ID[
            i]:
            corr_distributions['list_of_corr_MZ_shen'].append(
                corr_network['n0_r1xr2'][i][j])
        elif i == j:
            corr_distributions['list_of_corr_SI_shen'].append(
                corr_network['n0_r1xr2'][i][j])
        else:
            corr_distributions['list_of_corr_nonrelated_shen'].append(
                corr_network['n0_r1xr2'][i][j])
...
for i in range(246, 380):
    for j in range(0, len(corr_network['n0_r1xr2'])):
        if [int(ID_individuals[i]), int(ID_individuals[j])] == twin_pairs_ID[
            i]:
            corr_distributions['list_of_corr_DZ_shen'].append(
                corr_network['n0_r1xr2'][i][j])
        elif i == j:
            corr_distributions['list_of_corr_SI_shen'].append(
                corr_network['n0_r1xr2'][i][j])
...
np.savez('./../outputs/corr_distributions/list_of_corr_SI_shen_n0',
         list=corr_distributions['list_of_corr_SI_shen'])
np.savez('./../outputs/corr_distributions/list_of_corr_MZ_shen_n0',
         list=corr_distributions['list_of_corr_MZ_shen'])
np.savez('./../outputs/corr_distributions/list_of_corr_DZ_shen_n0',
         list=corr_distributions['list_of_corr_DZ_shen'])
np.savez('./../outputs/corr_distributions/list_of_corr_nonrelated_shen_n0',
         list=corr_distributions['list_of_corr_nonrelated_shen'])
...
for j in range(1, number_of_networks_shen + 1):
    fun.network(eval('ns.network' + str(j)), shen_matrices_r1,
                matrices_network_r1['network' + str(j)])
    fun.network(eval('ns.network' + str(j)), shen_matrices_r2,
    ...
    corr_distributions['list_of_corr_MZ_shen_n' + str(j)] = []
    corr_distributions['list_of_corr_DZ_shen_n' + str(j)] = []
    corr_distributions['list_of_corr_nonrelated_shen_n' + str(j)] = []
    corr_distributions['list_of_corr_SI_shen_n' + str(j)] = []
    ...
    for i in range(0, 246):
        for l in range(0, len(corr_matrices['n' + str(j) + '_SI'][0])):
            if [int(ID_individuals[i]), int(ID_individuals[l])] == \
                    twin_pairs_ID[i]:
                corr_distributions[
                    'list_of_corr_MZ_shen_n' + str(j)].append(
                    corr_matrices['n' + str(j) + '_SI'][0][i][l])
            elif i == l:
                corr_distributions[
                    'list_of_corr_SI_shen_n' + str(j)].append(
                    corr_matrices['n' + str(j) + '_SI'][0][i][l])
            else:
                corr_distributions[
                    'list_of_corr_nonrelated_shen_n' + str(j)].append(
                    corr_matrices['n' + str(j) + '_SI'][0][i][l])
    ...
    for i in range(246, 380):
        for l in range(0, len(corr_matrices['n' + str(j) + '_SI'][0])):
            if [int(ID_individuals[i]), int(ID_individuals[l])] == \
                    twin_pairs_ID[i]:
                corr_distributions[
                    'list_of_corr_DZ_shen_n' + str(j)].append(
                    corr_matrices['n' + str(j) + '_SI'][0][i][l])
            elif i == l:
                corr_distributions[
                    'list_of_corr_SI_shen_n' + str(j)].append(
...
for k in range(1, number_of_networks_shen + 1):
    np.savez('./../outputs/corr_distributions/list_of_corr_SI_shen_n' + str(k),
             list=corr_distributions['list_of_corr_SI_shen_n' + str(k)])
    np.savez('./../outputs/corr_distributions/list_of_corr_MZ_shen_n' + str(k),
             list=corr_distributions['list_of_corr_MZ_shen_n' + str(k)])
    np.savez('./../outputs/corr_distributions/list_of_corr_DZ_shen_n' + str(k),
             list=corr_distributions['list_of_corr_DZ_shen_n' + str(k)])
    np.savez(
        './../outputs/corr_distributions/list_of_corr_nonrelated_shen_n' + str(k),
        list=corr_distributions['list_of_corr_nonrelated_shen_n' + str(k)])

np.savez('./../outputs/accuracies_ind_id_shen.npz', dict=identification_accuracy)
np.savez('./../outputs/accuracies_perm_ind_id_shen.npz', dict=accuracies_random)


for j in range((len(identification_accuracy))):
    print('Network'+str(j) +'=' + str(np.sum(accuracies_random['n' + str(j) + '_SI']>np.mean(identification_accuracy['n' + str(j) + '_SI']))))
