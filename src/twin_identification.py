#!/usr/bin/env python3
"""
Code for twin identification based on functional connectivity
"""
import numpy as np
...
# TODO: Save heatmap to output directory
# Heatmap
...
for j in range(1, number_of_networks_finn + 1):
    ...
    for i in range(0, len(eval('corr_matrices_n' + str(j)))):
        eval('accuracies_n' + str(j) + '_MZ').append(
            accuracy_t_MZ(eval('corr_matrices_n' + str(j))[i][0:246, :], ID_individuals, tid.twin_pairs_ID))
    print
    eval('accuracies_n' + str(j) + '_MZ')
    ...
    for k in range(0, 1000):
        ...
        for i in range(0, len(eval('corr_matrices_n' + str(j)))):
            accuracies_MZ.append(
                accuracy_t_MZ(eval('corr_matrices_n' + str(j))[i][0:246, :], ID_individuals, np.ndarray.tolist(
                    np.random.permutation(tid.twin_pairs_ID[:246]))))
        eval('accuracy_random_MZ_n' + str(j)).append(np.mean(accuracies_MZ))

        for i in range(0, len(corr_matrices_whole_DZ)):
            accuracies_DZ.append(
                accuracy_t_DZ(eval('corr_matrices_n' + str(j))[i][246:, :], ID_individuals, np.ndarray.tolist(
                    np.random.permutation(tid.twin_pairs_ID[246:]))))
